		...
		<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
			...
			<td class="discount_minimum">
				{if $discount.minimal == 0}
					{l s='none'}
				{else}
					{$discount.minimal|escape:'htmlall':'UTF-8'}
				{/if}
			</td>
...
<p>
	<a href="{$base_dir}my-account.php" title="{l s='Back to Your Account'}"><img src="{$img_dir}icon/my-account.gif" alt="{l s='Back to Your Account'}" class="icon" />{l s='Back to Your Account'}</a>
	<a href="{$base_dir}" title="{l s='Home'}"><img src="{$img_dir}icon/home.gif" alt="{l s='Home'}" class="icon" />{l s='Home'}</a>
</p>