{capture name=path}{l s='Return Merchandise Authorization (RMA)'}{/capture}
...
<div class="block-center" id="block-history">
	...
	<table id="order-list" class="std">
		<thead>
			<tr>
				<th class="first_item">{l s='Return ID'}</th>
				<th class="item">{l s='Order ID'}</th>
		...
		{foreach from=$ordersReturn item=return name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				<td class="bold"><a class="color-myaccount" href="javascript:showOrder(0, {$return.id_order_return|intval}, 'order-return');">{l s='#RE'}{$return.id_order_return|string_format:"%06d"}</a></td>
				<td class="history_method"><a class="color-myaccount" href="javascript:showOrder(1, {$return.id_order|intval}, 'order-detail');">{l s='#FA'}{$return.id_order|string_format:"%06d"}</a></td>
				...
				{if $return.state == 2}
					<a href="{$base_dir}pdf-order-return.php?id_order_return={$return.id_order_return|intval}" title="{l s='Order return'} {l s='#RE'}{$return.id_order_return|string_format:"%06d"}"><img src="{$img_dir}icon/pdf.gif" alt="{l s='Order return'} {l s='#RE'}{$return.id_order_return|string_format:"%06d"}" class="icon" /></a>
					<a href="{$base_dir}pdf-order-return.php?id_order_return={$return.id_order_return|intval}" title="{l s='Order return'} {l s='#RE'}{$return.id_order_return|string_format:"%06d"}">{l s='Print out'}</a>
				{else}
					--
				{/if}
				</td>
